...
public class Kernel {
	...
	private JSONArray evaluate_inner(KernelLink kernelLink, String dir, JSONArray scripts) throws IOException, MathLinkException, JDOMException {
		...
		outer: for (int i = 0; i < scripts.size(); i++) {
			...
			for (int j = 0; j < json.size(); j++) {
				...
				} else if(obj.getString("type").equals("x3d")) {
					String x3d = config.getDir().getTemp() + obj.getString("data").replaceAll("\n", "") + ".x3d";
					Document document = builder.build(new File(x3d));
					Element scene = document.getRootElement().getChild("Scene");
					List<Element> shapes = scene.getChildren("Shape");
					shapes.forEach(shape -> {
						boolean append = false;
						pointSet:for(Element e : shape.getChildren()) {
							if(e.getName().equalsIgnoreCase("PointSet")) continue pointSet;
							append = true;
						}
						if(append && shape.getChild("Appearance") == null){
							shape.addContent(new Element("Appearance").addContent(new Element("ImageTexture").setAttribute("url", "../texture.jpg")));
						}
					});
					try(FileOutputStream out = new FileOutputStream(x3d)){
